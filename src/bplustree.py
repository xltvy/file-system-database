# B+ Tree object implementation is taken from https://gist.github.com/benben233/2c8a2a8ab44a7beabad0df1b6658232e
# 
# BPlusTree object methods "leafs" and "serialize" besides methods in this file "range" and "tree_serializer"
# are written by Altay Acar and Engin Oguzhan Senol.
...
class BPlusTree(object):
    ...
    def __setitem__(self, key, value, leaf=None):
        ...
        if leaf is None:
            leaf = self.find(key)
        leaf[key] = value
        #Insert the key-value pair into json file
    ...
    def leafs(self, node=None, file=None, _prefix="", _last=True, _list=[]): 
        if node is None:
            node = self.root
        if type(node) is Leaf:
            _list.extend(node.keys)
        if type(node) is Node:
            for i, child in enumerate(node.values):
                _last = (i == len(node.values) - 1)
                self.leafs(child, file, _prefix, _last, _list)
        return _list

    def serialize(self, node=None, file=None, _prefix="", _last=True, _dict={"Non-Leafs":[], "Leafs":[]}): 
        if node is None:
            node = self.root
        if type(node) is Leaf:
            _dict["Leafs"].append(node.keys)
        else:
            _dict["Non-Leafs"].append(node.keys)
        if type(node) is Node:
            for i, child in enumerate(node.values):
                _last = (i == len(node.values) - 1)
                self.serialize(child, file, _prefix, _last, _dict)
        return _dict
...
def range(bplustree, operation, parameter):
    keys = bplustree.leafs()
    result_keys = []
    if operation == ">":
        for key in keys:
            if key > parameter:
    ...
    if len(result_keys) > 0:
        ...
        for key in result_keys:
            result_vals.append(bplustree.query(key))
        return result_vals
    else:
        return None
    
def tree_serializer(bplustree):
    result_dict = {"Non-Leafs":[], "Leafs":[]}
    init_dict = bplustree.serialize()
    nonleaf_count = 0
    for node in init_dict["Non-Leafs"]:
        node_dict = {}
        if nonleaf_count == 0:
            node_dict["Root"] = node
        else:
            key = "Inner Node " + str(nonleaf_count)
            node_dict[key] = node
        result_dict["Non-Leafs"].append(node_dict)
        nonleaf_count = nonleaf_count + 1
    leaf_count = 1
    for node in init_dict["Leafs"]:
        fin_dict = {}
        leaf_key = "Leaf " + str(leaf_count)
        leaf_dict = {}
        for data in node:
            value = bplustree.query(data)
            leaf_dict[data] = value
        fin_dict[leaf_key] = leaf_dict
        result_dict["Leafs"].append(fin_dict)
